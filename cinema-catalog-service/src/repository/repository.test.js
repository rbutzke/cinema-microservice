...
function runTests() {
  ...
  test("Repository getCinemasByCityId", (t) => {
    repository.getCinemasByCityId(cityId, (err, cinemas) => {
      ...
      t.assert(
        !err && cinemas && cinemas.length > 0,
        "All Cinemas Returned By City Id"
      );
      t.end();
    });
  });
  ...
  test("Repository getMoviesByCinemaId", (t) => {
    repository.getMoviesByCinemaId(cinemaId, (err, movies) => {
      t.assert(
        !err && movies && movies.length > 0,
        "Movies By Cinema Id Returned"
      );
      t.end();
    });
  });
  ...
  test("Repository getMovieSessionsByCinemaId", (t) => {
    repository.getMovieSessionsByCinemaId(
      movieId,
      cinemaId,
      (err, sessions) => {
        t.assert(
